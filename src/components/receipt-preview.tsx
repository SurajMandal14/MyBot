...
interface ReceiptPreviewProps {
  receiptData: ReceiptSchema | null;
}

export function ReceiptPreview({ receiptData }: ReceiptPreviewProps) {
  const [currentDate, setCurrentDate] = useState('');

  const grandTotal = receiptData?.items.reduce((acc, item) => acc + (item.total || 0), 0) || 0;
    ...
    return (
      <div className="flex h-full items-center justify-center bg-gray-50 text-gray-400 rounded-sm">
        <div className="text-center">
            <p className="font-headline text-lg">Your receipt will appear here</p>
  ...
  return (
    <div id="receipt-print-area" className="relative bg-white text-black p-8 font-body text-[10px] w-full h-full overflow-auto">
        <div className="relative z-10 flex flex-col h-full">
            ...
            <section className="grid grid-cols-3 gap-4 text-xs mb-4">
                ...
                <div>
                    ...
                    <p className="text-gray-800 font-medium">{formatCustomerName(receiptData.customerName)}</p>
                </div>
                <div>
            ...
            <section className="text-xs mb-4">
                ...
                <p className="text-gray-800">{receiptData.carModel || 'N/A'}</p>
                <p className="text-gray-800">{receiptData.vehicleNumber || 'N/A'}</p>
            </section>
