...
import { quotationSchema, type QuotationSchema } from '@/lib/validators';
import { useToast } from '@/hooks/use-toast';
import { modifyInvoiceAction, parseQuotationAction } from '@/app/actions';
...
const initialQuotationState: QuotationSchema = {
  quotationNumber: '',
  customerName: '',
  vehicleNumber: '',
  carModel: '',
...
export function QuotationCreator() {
  ...
  const form = useForm<QuotationSchema>({
    ...
    defaultValues: initialQuotationState,
    mode: 'onChange'
  });
  ...
  const handleParse = () => {
    ...
    startParsing(async () => {
      const result = await parseQuotationAction({ text: rawText });
      if (result.success && result.data) {
        form.reset(result.data);
        setIsQuotationReady(true);
        toast({ title: 'Parsing Successful', description: 'Quotation details have been extracted.' });
      } else {
  ...
  const handleAiModify = () => {
    ...
    if (!isQuotationReady) {
      toast({ title: 'No Quotation Data', description: 'Parse a quotation before modifying it.', variant: 'destructive' });
      return;
    }

    startModifying(async () => {
      const currentQuotationString = JSON.stringify(quotationData);
      const result = await modifyInvoiceAction({ documentDetails: currentQuotationString, modificationRequest: aiCommand });
      if (result.success && result.data) {
        const parsedResult = result.data as QuotationSchema;
          ...
          {isQuotationReady && (
            <Form {...form}>
              <form>
                <Card>
                  <CardHeader>
                    <CardTitle className="font-headline">Edit Quotation Details</CardTitle>
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="rounded-md border">
                       ...
                       <Button type="button" variant="secondary" size="sm" className="mt-4" onClick={() => append({ description: '', total: 0 })}>
        ...
        <div className="lg:sticky top-8">
          <Card className="shadow-lg">
            <CardHeader className="flex-row items-center justify-between no-print">
              <div>
                <CardTitle className="font-headline">Quotation Preview</CardTitle>
                <CardDescription>This is how your quotation will look.</CardDescription>
              </div>
              <div className="flex gap-2">
                <Button onClick={handlePrint} disabled={!isQuotationReady} variant="outline" size="sm">
            ...
            <CardContent>
              <div className="aspect-[8.5/11] bg-white rounded-md border p-2">
                <QuotationPreview quotationData={isQuotationReady ? quotationData : null} />
              </div>
            </CardContent>
          </Card>
        </div>
